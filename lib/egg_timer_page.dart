...
import 'package:superdeclarative/_reorienting_gesture_detector.dart';
...
class _EggTimerDialState extends State<EggTimerDial>
  ...
  Offset _mapDragPointToCartesianOrientation(Offset localOffset) {
    const eggTimerOrientation = EggTimerOrientation();
    return localOffset
        .toPoint()
  ...
  Widget _buildInnerKnob(Angle angle) {
    return Container(
      ...
      child: Container(
        ...
        child: Center(
          child: Transform(
            transform: Matrix4.rotationZ(angle.radians.toDouble()),
...
class TickPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < tickCount; ++i) {
      ...
      // example of where not having a full circle rotation is annoying
      // Ideally:
...
class ArrowPainter extends CustomPainter {
  ArrowPainter({required this.angle}) : dialArrowPaint = Paint() {
...
class EggTimer with ChangeNotifier {
  ...
  void setTimer(Duration countdown) {
    if (state == _EggTimerState.ready) {
      ...
      debugPrint('Cannot set the timer when it is running or paused.');
    }
  }
  ...
  void start() {
    if (state == _EggTimerState.ready) {
      ...
      debugPrint('Cannot start an EggTimer that is already running.');
    }
  }
  ...
  void pause() {
    if (state == _EggTimerState.running) {
      ...
      debugPrint('Cannot pause an EggTimer that is not running.');
    }
  }
  ...
  void resume() {
    if (state == _EggTimerState.paused) {
      ...
      debugPrint('Cannot resume an EggTimer that is not paused.');
    }
  }
  ...
  void restart() {
    if (state == _EggTimerState.paused) {
      ...
      debugPrint('Cannot restart an EggTimer that is not paused.');
    }
  }
  ...
  void reset() {
    if (state == _EggTimerState.paused) {
      ...
      debugPrint('Cannot reset an EggTimer that is not paused.');
    }
  }
